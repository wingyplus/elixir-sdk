...
The time to watch is the `elixir-sdk:generate-all` span, which `sweep.sh`
prints per module count; the wall time around it is mostly CLI start-up and
loading the workspace's own modules, and does not grow with the module count.
